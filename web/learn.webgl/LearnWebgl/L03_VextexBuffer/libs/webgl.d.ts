...
interface WebGLRenderingContext {
	...
	/**
	 * 设置 顶点shader的属性变量数据
	 * @param indx 属性指针； 使用这个函数可以绑定 shader属性变量 gl.bindAttribLocation(programObject, positionIndex, "position"); 
	 * @param size 设置该属性变量类型值的大小； 如：attribute vec3 position; vec3占3个数值所以是3
	 * @param type 数据类型； 如: FLOAT
	 * @param normalized 是否归一化
	 * @param stride 每个顶点占数组中多少个字节； 如果是float32，那么单个维度值是4个字节；如果只有xyz信息那么就是 4 * 3；如果是xyzrgb 就是 $ * 6;如果是xyzrgbuv就是4*8
	 * @param offset 偏移数组中多少个字节；计算方式和stride是一样的；如要要跳过前2个顶点,一个顶点的信息有xyzrpg,那么计算是 4 * 6 * 2
	 */

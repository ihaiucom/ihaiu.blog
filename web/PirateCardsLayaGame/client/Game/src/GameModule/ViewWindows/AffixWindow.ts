...
import AffixWindowUI from '../../FGUI/Extends/ModuleDialogs/AffixWindowUI';


export default class AffixWindow extends MWindow
{
    window: AffixWindowUI;
    constructor()
    {
        super();

        // 添加依赖资源
        this.addAssetForFguiComponent(AffixWindowUI);
    ...
    {
        let windowUI = AffixWindowUI.createInstance();
    ...
    onMenuOpen(parameter: MenuOpenParameter): void  {
        ...
        {
            let args = parameter.args;
            this.window.ShowTips(args[0], args[1]);
        }
        else
        {
        }

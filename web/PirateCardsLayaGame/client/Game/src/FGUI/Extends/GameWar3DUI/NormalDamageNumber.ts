...
import NormalDamageNumberStruct from "../../Generates/GameWar3DUI/NormalDamageNumberStruct";

export default class NormalDamageNumber extends NormalDamageNumberStruct
{
    private static pool = [];
    static poolGet():NormalDamageNumber
    {
        let item:NormalDamageNumber;
        ...
        {
            item = NormalDamageNumber.createInstance();
        }
        item.visible = true;
    ...
    poolRecover()
    {
        if(NormalDamageNumber.pool.indexOf(this) == -1 )
        {
            NormalDamageNumber.pool.push(this);
        }
    }
    ...
    init(num : number, x : number, y : number, path:any, dir:number)
    {
        ...
        Tween.to(this, { y: this.startY - 80,alpha: 0.0 }, 400, Ease.linearNone, Laya.Handler.create(this, this.release), 448, false);
    }
    /**
     * 设置伤害数值
     * @param value 伤害的数字大小

...
def getFixTypeValue(jsontype,jsonvalue):
	...
	if IsStringType(jsontype):
		...
		try:
			...
			jsonvalue= str(int(float(jsonvalue)))
	else:
		jsonvalue= str(jsonvalue)
	return jsonvalue

def getFixTypeJsonListStr(jsonname,jsontype,jsonvalue,col):
	if isinstance(jsonvalue,float):
		if jsonvalue % 1 == 0.0:
			jsonvalue = int(jsonvalue)
	data = str(jsonvalue)
	...
	for index, value in enumerate(valueList):
		if IsNumberType(jsontype) and value.find("&")>=0:
			print(" type error: type={0}[],value={1},row:{2},col:{3}".format(jsontype,jsonvalue,1,col+1))
			os.system("pause")
...
def getSelfDefTypeJsonStr(structName,Fields,Types,jsonname,data,row,col):
	...
	if len(valueList) != len(Fields):
		print("parse {0} failure,field length != {1},data:{2},row:{3},col:{4}".format(structName,len(Fields),data,row+1,col+1))
		os.system("pause")
...
def getSelfDefTypeJsonListStr(structName,Fields,Types,jsonname,data,row,col):
	if isinstance(data,float):
		if data % 1 == 0.0:
			data = int(data)
	data = str(data)
	...
	for i, struct in enumerate(structList):
		...
		if len(valueList) != len(Fields):
			print("parse {0} list failure,field length != {1},data:{2},row:{3},col:{4}".format(structName,len(Fields),data,row+1,col+1))
			os.system("pause")
		jsonStr += "{"
		for index, value in enumerate(valueList):
			fieldValue = getFixTypeValue(Types[index],value)
	...
	for sheetname in workbook.sheet_names():
		...
		for rownum in range(4, nrows):
			...
			if len(fields) != len(fieldTypes):
				print("parse ExportSetting error struct:{0},fiels length is not equal to types length,row:{1}".format(structName,rownum+1))
				os.system("pause")
				return selfDefInfo
			newFields = []
			newTypes = []
	...
	for rownum in range(0, nrows):
		...
		if jsontype.endswith("[]"):
			...
			if tmpType in selfDefInfo:
				tmpStr = getSelfDefTypeJsonListStr(tmpType,selfDefInfo[tmpType][0],selfDefInfo[tmpType][1],jsonname,jsonvalue,rownum,0)
				if tmpStr != "":
					if(rownum != 0):
						jsonLine = jsonLine + ","
					jsonLine += tmpStr
			elif tmpType in baseTypeList:
				tmpStr = getFixTypeJsonListStr(jsonname,tmpType,jsonvalue,rownum)
				if tmpStr != "":
					if(rownum != 0):
						jsonLine = jsonLine + ","
					jsonLine += tmpStr
			else:
				print("type error:global can not find type:{0},row:{1}".format(jsontype,rownum+1))
				os.system("pause")
		elif jsontype in selfDefInfo:
			tmpStr = getSelfDefTypeJsonStr(jsontype,selfDefInfo[jsontype][0],selfDefInfo[jsontype][1],jsonname,jsonvalue,rownum,0)
			if tmpStr != "":
				if(rownum != 0):
					jsonLine = jsonLine + ","
				jsonLine += tmpStr
			...
			print("type error:can not find type:{0},row:{1}".format(jsontype,rownum+1))
			os.system("pause")
...
def ParseExcelToJson(filename,excelname,selfDefInfo):
	...
	for colnum in range(0, ncols):
		...
		if jsonname in idCheck:
			print("key repeat error:key:{0}, row:3,col:{1}".format(jsonname,colnum))
			os.system("pause")
		else:
			idCheck[jsonname] = colnum
	for rownum in range(4, nrows):
		key = ""
		clientJsonLine = ""
		serverJsonLine = ""
		for colnum in range(0, ncols):
			...
			if parseType == "":
				print("parse type error:nil row:4,col:{0}".format(colnum+1))
				os.system("pause")
			...
			if str(jsonvalue) == "":
				continue
			if jsonname == "":
				print("parse key error:nil row:3,col:{0}".format(colnum+1))
				os.system("pause")
			if jsontype == "":
				print("type error:nil row:1,col:{0}".format(colnum+1))
				os.system("pause")
			...
			if jsontype.endswith("[]"):
				...
				if tmpType in type_map:
						tmpType = type_map[tmpType]
				if tmpType in selfDefInfo:
					tmpStr = getSelfDefTypeJsonListStr(tmpType,selfDefInfo[tmpType][0],selfDefInfo[tmpType][1],jsonname,jsonvalue,rownum,colnum)
					if tmpStr != "":
						if(colnum != 0):
							jsonLine = jsonLine + ","
						jsonLine += tmpStr
				elif tmpType in baseTypeList:
					tmpStr = getFixTypeJsonListStr(jsonname,tmpType,jsonvalue,colnum)
					if tmpStr != "":
						if(colnum != 0):
							jsonLine = jsonLine + ","
						jsonLine += tmpStr
				else:
					print("type error:can not find type:{0}, row:1,col:{1}".format(jsontype,colnum+1))
					os.system("pause")
			elif jsontype in selfDefInfo:
				tmpStr = getSelfDefTypeJsonStr(jsontype,selfDefInfo[jsontype][0],selfDefInfo[jsontype][1],jsonname,jsonvalue,rownum,colnum)
				if tmpStr != "":
					if(colnum != 0):
						jsonLine = jsonLine + ","
					jsonLine += tmpStr
		...
		if key == "":
			print("no id error:can not find id (row:3,col:1)")
			os.system("pause")
		if(rownum != 4):
			clientJsonLine = ",\n\t\t"+key+":{" + clientJsonLine + "}"
			serverJsonLine = ",\n\t\t"+key+":{" + serverJsonLine + "}"
